...
} from "@/components/awaiting-review/awaiting-review-modal";
...
function compareBadges(a: string | null | undefined, b: string | null | undefined): number {
 const left = getBadgeDefinition(a);
 const right = getBadgeDefinition(b);
 const leftIndex = left ? MOVIE_BADGES.findIndex((badge) => badge.id === left.id) : Number.POSITIVE_INFINITY;
 const rightIndex = right ? MOVIE_BADGES.findIndex((badge) => badge.id === right.id) : Number.POSITIVE_INFINITY;
...
function compareBySortKey(
 ...
 case "badge":
 return compareBadges(a.badgeId, b.badgeId);
 case "stars":
...
function isMissingSortValue(movie: LibraryMovie, key: LibrarySortKey): boolean {
 ...
 case "badge":
 return !getBadgeDefinition(movie.badgeId);
 case "stars":
...
export function LibraryShell() {
 ...
 const [isAwaitingReviewOpen, setIsAwaitingReviewOpen] = useState(false);
 const [awaitingReviewItems, setAwaitingReviewItems] = useState<
 AwaitingReviewItem[]
 >([]);
 ...
 try {
 const response = await fetch("/api/awaiting-review", { cache: "no-store" });
 const data = (await response.json()) as
 | { ok: true; items: AwaitingReviewItem[] }
 | { ok: false; error: string };

 if (response.ok && data.ok) {
 setAwaitingReviewItems(data.items);
 }
 } catch {
 }
 }, []);
 ...
 useEffect(() => {
 loadAwaitingReview();
 window.addEventListener("hma-awaiting-review-updated", loadAwaitingReview);

 return () => {
 window.removeEventListener("hma-awaiting-review-updated", loadAwaitingReview);
 };
 }, [loadAwaitingReview]);

 const handleOpenAwaitingReview = useCallback(() => {
 loadAwaitingReview();
 setIsAwaitingReviewOpen(true);
 }, [loadAwaitingReview]);
 ...
 () => ({
 decades: uniqueSorted(movies.map(getMovieDecade)).sort(
 ...
 onOpenAwaitingReview={handleOpenAwaitingReview}
 ...
 <AwaitingReviewModal
 open={isAwaitingReviewOpen}
 onOpenChange={setIsAwaitingReviewOpen}
 items={awaitingReviewItems}
 onQueueChange={setAwaitingReviewItems}
 />
